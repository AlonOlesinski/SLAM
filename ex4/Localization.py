...
class TrackDB:
    def __init__(self):
        self.frames_dict = {}  # value: dict of track_id: track
    ...
    def get_frame_tracks(self, frame_id):
        ...
        """
        return self.frames_dict[frame_id].keys()
    ...
    def get_track_frames(self, track_id):
        ...
        """
        frame = track_id[0]
        return self.frames_dict[frame][track_id].get_frames()
    ...
    def add_match(self, l0_id: int, x_l0: int, y_l0: int, x_r0: int, y_r0: int,
        ...
        """
        if l0_id not in self.frames_dict.keys():
            self.frames_dict[l0_id] = {}
        if l1_id not in self.frames_dict.keys():
            self.frames_dict[l1_id] = {}
        ...
        if (x_l0, y_l0) in self.last_added_tracks.keys():
            ...
            self.frames_dict[l0_id][track.__id__()] = track

        track.add_location((x_l1, y_l1), (x_r1, y_r1))
        self.frames_dict[l1_id][track.__id__()] = track
    ...
    def get_location(self, frame_id, track_id):
        ...
        """
        track = self.frames_dict[frame_id][track_id]
    ...
    def get_all_unique_tracks(self):
        ...
        """
        tracks = []
        for track_dict in self.frames_dict.values():
    ...
    def serialize(track_db, filename):
        ...
        with open(filename, 'wb') as f:
            pickle.dump([track_db.frames_dict, track_db.inliers_outliers_count], f)
    ...
    def deserialize(filename):
        ...
        with open(filename, 'rb') as f:
            ...
            track_db.frames_dict = loaded_list[0]
    ...
    def get_statistics(self):
        ...
        num_frames = len(self.frames_dict.keys())
        mean_num_frame_links = np.mean([len(self.frames_dict[k]) for k in self.frames_dict.keys()])
    ...
    def get_frame_connectivity(self, frame_id):
        ...
        """
        track_dict = self.frames_dict[frame_id]
        count = 0
        for track in track_dict.values():
            track_first_frame_id = track.first_frame_id
            if frame_id - track_first_frame_id >= len(track) - 1:
                count += 1
    ...
    def get_track_with_len_at_least_10(self):
        all_tracks = self.get_all_unique_tracks()
        for track in all_tracks:
            if len(track.locations) == 11 and track.first_frame_id == 67:
                return track
...
class Localizer:
    ...
    def __init__(self, frame0_index, track_db, detector, matcher, left0, right0, left1, right1, left0_camera,
                 right0_camera, intrinsic_matrix,
...
def create_track_db():
    ...
    for i in range(ITERATIONS):
        ...
        localizer = Localizer(i, track_db, feature_detector, bf, left0, right0, left1, right1, left0_camera,
                              right0_camera, k,
    ...
    TrackDB.serialize(track_db, 'track_db.pkl')
...
def run_4_2():
    try:
        track_db = TrackDB.deserialize('track_db.pkl')
