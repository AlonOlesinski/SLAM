...
trajectory \ from a sequence of stereo image pairs.
The pipeline is evaluated on sequences of the [KITTI](https://www.cvlibs.net/datasets/kitti/) odometry dataset, using the provided ground-truth poses for quantitative validation.
...
The figure above shows the recovered trajectory (top-down view) for each stage compared to ground truth. PnP drifts noticeably; BA tightens the local geometry; loop closure brings the global trajectory back onto the ground-truth path.
...
Most matches end up as supporters of the recovered pose, confirming that the geometric estimate is consistent with the data. Composing these relative poses gives a global trajectory, but small errors accumulate frame-to-frame and the path drifts over the sequence.
...
[GTSAM](https://gtsam.org/). The sequence is split into overlapping **bundles** whose length is chosen adaptively from the median track length starting at each keyframe
...
- a `GenericStereoFactor3D` for every stereo observation (combining the left and right pixel measurements), and
...
<img src="project/results/median_projection_error_per_keyframe.png" title="" alt="Median reprojection error per keyframe before/after BA" width="608">
...
1. Candidates are scored by the Mahalanobis distance between the current keyframe's estimated pose and earlier keyframes (covariance is summed along the shortest path in a parallel covariance graph maintained with Dijkstra).
2. The top candidates are verified by **consensus matching** - running the PnP+RANSAC localizer between the two keyframes and checking the inlier percentage.
...
After every accepted loop closure the pose graph is re-optimized with Levenberg–Marquardt, which redistributes accumulated drift across all keyframes in the
loop.
